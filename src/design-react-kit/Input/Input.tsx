...
export const Input = ({
  ...
  useEffect(() => {
    if (
      divResizeRef.current != null &&
      divResizeRef.current.classList.contains("input-number-adaptive")
    ) {
      if (!value) {
        setWidth(`calc(70px)`);
      } else {
  ...
  const validationTextControlClass = getValidationTextControlClass(
    { valid },
    cssModule
  );
  ...
  const clickIncrDecr = (mode: number) => {
    ...
    const min = parseFloat(
      inputRef.current?.min ? inputRef.current.min : "Nan"
    );
    const max = parseFloat(
      inputRef.current?.max ? inputRef.current.max : "Nan"
    );
    step = isNaN(step) ? 1 : step;
    const newValue =
      parseFloat(inputRef.current?.value ? inputRef.current.value : "0") +
      mode * step;
